...
$(document).ready(function () {
  let isPlaying = false; // Track if the flipbook is currently playing
  let autoFlipTimeout; // Store the timeout ID for auto-flipping

  function adjustBookSize() {
    var width = $(window).width() - 100;
    var height = $(window).height() - 100;

    $(".pages").turn("size", width, height);

    // Center the pages element
    $(".pages").css({
      position: "absolute",
      left: "50%",
      top: "50%",
  ...
  $(window).on("resize", adjustBookSize);
  ...
  function updateButtons() {
    var currentPage = $(".pages").turn("page");
    var totalPages = $(".pages").turn("pages");

    $("#prev-page").prop("disabled", currentPage === 1);
    $("#next-page").prop("disabled", currentPage === totalPages);
    $("#play").prop("disabled", currentPage === totalPages);
  }

  // Function to automatically flip through all pages
  function autoFlip() {
    if (!isPlaying) return; // Stop if playing is disabled
    ...
    if (currentPage < totalPages) {
      ...
      // Continue flipping until the last page
      autoFlipTimeout = setTimeout(function () {
        autoFlip();
      }, 1500); // Delay between flips (same as the flip duration)
    } else {
      stopAutoFlip(); // Stop when the last page is reached
    }
  }
  ...
  function stopAutoFlip() {
    isPlaying = false;
    clearTimeout(autoFlipTimeout); // Clear the timeout
    $("#play").show(); // Show the Play button
